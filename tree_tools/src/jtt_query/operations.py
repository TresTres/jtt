...
class GetAll(QueryOperation):
    ...
    def evaluate(self, node: jtt_tree.TreeNode) -> typing.List[jtt_tree.TreeNode]:
        ...
        if self.strict:
            raise QueryOperationError("Cannot evaluate query get-all operation on non-array or non-object node.")
        return []
...
class FilterIndex(QueryOperation):
    ...
    def evaluate(self, node: jtt_tree.TreeNode) -> typing.List[jtt_tree.TreeNode]:
        if node.type is jtt_tree.NodeType.ARRAY:
            ...
            if self.strict: 
                raise QueryOperationError(f"Index {self.index} out of bounds for array node with length {len(node.value)}.")
            return []
        if self.strict:
            raise QueryOperationError("Cannot evaluate query index-matching operation on non-array node.")
        return []
...
class FilterKey(QueryOperation):
    ...
    def evaluate(self, node: jtt_tree.TreeNode) -> typing.List[jtt_tree.TreeNode]:
        ...
        if node.type is jtt_tree.NodeType.OBJECT:
            ...
            if self.strict:
                raise QueryOperationError(f"Key {self.predicate} not found in object node with keys {node.value.keys()}.")
            return []
        if self.strict:
            raise QueryOperationError("Cannot evaluate query key-matching operation on non-object node.")
        return []
